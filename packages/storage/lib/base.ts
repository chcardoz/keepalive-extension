 ...
 * @enum {string}
 *
 * @property {string} Local - Persist data locally against browser restarts. Will be deleted by uninstalling the extension.
 * @default
 *
 * @property {string} Sync - Uploads data to the user's account in the cloud and syncs to the user's browsers on other devices. Limits apply.
 *
 * @property {string} Managed - Requires an [enterprise policy](https://www.chromium.org/administrators/configuring-policy-for-extensions) with a JSON schema for company-wide config.
 *
 * @property {string} Session - Only persists data until the browser is closed. Recommended for service workers which can shut down anytime and therefore need to restore their state. Set {@link SessionAccessLevel} for permitting content scripts access.
 * @implements Chrome's [Session Storage](https://developer.chrome.com/docs/extensions/reference/storage/#property-session)
 */
 ...
 *
 * @implements Chrome's [Session Access Level](https://developer.chrome.com/docs/extensions/reference/storage/#method-StorageArea-setAccessLevel)
 *
 * @enum {string}
 *
 * @property {string} ExtensionPagesOnly - Storage can only be accessed by Extension pages (not Content scripts).
 * @default
 *
 * @property {string} ExtensionPagesAndContentScripts - Storage can be accessed by both Extension pages and Content scripts.
 */
